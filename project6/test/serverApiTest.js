...
// 添加新测试模块
describe("test /user/listWithCounts", function(done) {
  it("returns user list with photo and comment counts", function (done) {
    http.get(`/user/listWithCounts`, (response) => {
      // 验证响应
      ...
      response.on("end", () => {
        ...
        // 验证每个用户都有计数字段
        users.forEach(user => {
          assert.strictEqual(typeof user.photo_count, "number");
          assert.strictEqual(typeof user.comment_count, "number");
        });
...
describe("test /commentsOfUser/:id", function(done) {
  it("returns user's comments with photo info", function(done) {
    // 先获取一个用户ID
    http.get(`/user/list`, (response) => {
      let data = "";
      response.on("data", chunk => data += chunk);
      response.on("end", () => {
        const userId = JSON.parse(data)[0]._id;

        // 获取该用户评论
        http.get(`/commentsOfUser/${userId}`, (response) => {
          assert.strictEqual(response.statusCode, 200);

          let commentData = "";
          response.on("data", chunk => commentData += chunk);
          response.on("end", () => {
            ...
            comments.forEach(comment => {
              ...
              assert(comment.date_time);
              assert(comment.photo_id);
              assert(comment.file_name);
              assert(comment.photo_owner);
            });
